...
#ifndef _AFMotorEVIL_h_
#define _AFMotorEVIL_h_
...
#define MOTORLATCHEVIL 13
#define MOTORCLKEVIL 2
#define MOTORENABLEEVIL 10
#define MOTORDATAEVIL 9

class AFMotorEVILController
{
  public:
    AFMotorEVILController(void);
    void enable(void);
    friend class AF_DCMotorEVIL;
    void latch_tx(void);
    uint8_t TimerInitalized;
};

class AF_DCMotorEVIL
{
 public:
  AF_DCMotorEVIL(uint8_t motornum, uint8_t freq = DC_MOTOR_PWM_RATE);
  void run(uint8_t);
...
class AF_StepperEVIL {
 public:
  AF_StepperEVIL(uint16_t, uint8_t);
